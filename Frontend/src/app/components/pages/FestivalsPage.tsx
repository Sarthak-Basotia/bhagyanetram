    ...
    <div className="min-h-screen bg-[#FFFDF2] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-6">
          <h1 className="text-3xl md:text-4xl font-extrabold text-[#D35400] mb-2">Hindu Festival Calendar</h1>
          <p className="text-slate-600 mb-8 text-sm md:text-base">Panchang timings and major auspicious days</p>
          ...
          <div className="flex items-center justify-center gap-4 md:gap-6 mb-6">
            ...
            <span className="text-lg md:text-xl font-bold text-slate-800 w-40 md:w-48 text-center uppercase">
        ...
        {/* RESPONSIVE WRAPPER: Allows horizontal scrolling on mobile without crushing the grid */}
        <div className="w-full overflow-x-auto pb-6 custom-scrollbar">
          
          {/* The Calendar Container (Forced minimum width to preserve layout) */}
          <div className="min-w-[768px] border-[8px] md:border-[12px] border-[#0A0A9C] bg-white rounded-md shadow-2xl p-1 relative mx-auto">
            <div className="border-[4px] md:border-[6px] border-[#F4C542] p-1">
              <div className="border-[3px] md:border-[4px] border-[#E83C3C] p-1 md:p-2">
                
                {/* Header: Month Names */}
                <div className="bg-[#F9ED34] py-2 md:py-3 text-center mb-1">
                  <h2 className="text-[#D31F1F] text-2xl md:text-4xl lg:text-5xl font-black tracking-wider flex justify-around items-center">
                    <span className="w-1/3 text-left pl-4">{currentMonthData.hinduMonthNames.split('/')[0] || ''}</span>
                    <span className="font-sans w-1/3 text-center">{currentMonthData.westernMonthName}</span>
                ...
                <div className="grid grid-cols-7 border-l-2 border-t-2 border-[#0A0A9C]">
                  ...
                  {getEmptyCells(currentMonthData.days[0]?.dayOfWeek || "SUN").map((_, idx) => (
                    <div key={`empty-${idx}`} className="border-r-2 border-b-2 border-[#0A0A9C] min-h-[100px] md:min-h-[120px] bg-slate-50/50"></div>
                  ))}

                  {/* Actual Calendar Days */}
                  {currentMonthData.days.map((day: any) => (
                    <div key={day.date} className="border-r-2 border-b-2 border-[#0A0A9C] min-h-[100px] md:min-h-[120px] p-1 md:p-2 flex flex-col relative bg-white hover:bg-slate-50 transition-colors">
                      
                      <span className="text-[#D31F1F] font-black text-lg md:text-xl mb-1">{day.date}</span>
                      
                      {day.isMajorFestival || (day.festivalHindi && day.festivalHindi.length > 0) ? (
                        <div className="border-2 border-[#D31F1F] p-1 mt-auto mb-1 text-center bg-white overflow-hidden">
                          <div className="text-[#8B008B] font-bold text-xs md:text-sm leading-tight mb-1 truncate whitespace-normal">{day.festivalHindi}</div>
                          <div className="text-[#D31F1F] font-bold text-[10px] md:text-xs leading-tight uppercase truncate whitespace-normal">{day.festivalEnglish}</div>
                        </div>
                      ) : (
                        <div className="text-center mt-auto mb-1 md:mb-2">
